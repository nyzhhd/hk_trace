...
class YOLO(object):
    ...
    def detect_image(self, image, crop = False, count = False):
        ...
        # 如果找到了最大的“person”框
        if max_index != -1:
            c = top_label[max_index]
            box = top_boxes[max_index]
            score = top_conf[max_index]
            ...
            # 准备标签文本
            label = '{} {:.2f}'.format(self.class_names[int(c)], score)
            draw = ImageDraw.Draw(image)
            label_size = draw.textsize(label, font)
            label = label.encode('utf-8')
        ...
        return image,(left, top, right, bottom) if max_index != -1 else None
